...
const Step2 = () => {
  ...
  const handleChecked = () => {
    const allChecked = fields.reduce((acc, current) => {
      if (current.isChecked === false) {
        let obj = {
          id: current.id,
          checkbox: current.checkbox,
          inputField: current.inputField,
          isChecked: true,
          value: current.value,
        };
        acc = Object.assign(obj, ...acc);
        console.log(acc);
        return acc;
      }
      console.log(allChecked);
      // setFields(allChecked);
    }, []);
  };
  // console.log(fields);
...
{/* 
            <div className="flex gap-4 mt-2 mx-[35px]">
              <input type="checkbox" name="" onChange="" value="" />
              <button
                className="border-2 py-1 px-2 bg-orange-200"
